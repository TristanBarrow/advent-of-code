...
class WrappedRound {
    ...
    getAmountByColor = (color: Color) => {
        const filteredRounds = this.round.colorResults.filter(
            (res) => res.color === color
        );
...
class WrappedGame {
    ...
    maxAmountByColor = (color: Color) => {
        return this.game.rounds
            .map((r) => r.getAmountByColor(color))
            .reduce((a, b) => (a > b ? a : b));
    };
...
export const part2 = (input: string): string => {
    ...
    const result = games
        .map((game) => {
            return (
                game.maxAmountByColor("red") *
                game.maxAmountByColor("green") *
                game.maxAmountByColor("blue")
            );
        })
        .reduce((a, b) => a + b);
