...
class Part {
    ...
    constructor(value: number, start: Location | null, end: Location | null) {
        ...
        if (start.vertical !== end.vertical) throw new Error("start and end must be on the same line");
        this.value = value;
        this.start = start;
        this.end = end;
    }
    partIsOnLocation(location: Location) {
        ...
        return this.start.horizontal <= location.horizontal && this.end.horizontal >= location.horizontal;
    }
    getUID() {
...
class PartCatalog {
    ...
    getPartByLocation(location: Location): Part | null {
        return this.parts.find((part) => part.partIsOnLocation(location)) || null;
    }
}
type Location = {
    horizontal: number;
...
class Schematic {
    ...
    locationIsNearSymbol(location: Location) {
        for (let i = -1; i <= 1; i++) {
            for (let j = -1; j <= 1; j++) {
                const testLoc = Loc(location.vertical + i, location.horizontal + j);
    ...
    getValue(location: Location): SchemeValue {
        if (location.vertical < 0 || location.vertical >= this.data.length) return nilSchemeValue;
        if (location.horizontal < 0 || location.horizontal >= this.data[location.vertical].length)
    ...
    s.forEach((entry, location) => {
        if (entry.category === "digit") {
            ...
            if (s.isEndOfNumber(location)) {
                if (currentBufferIsValid) validNumbers.push(parseBuffer(buffer));
                buffer = [];
